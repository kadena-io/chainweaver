...
uiTextInputAsync
  ...
  -> (InputElement EventResult GhcjsDomSpace t -> Event t ())
  -- ^ This function is responsible for generating an event. This event controls when the validation is supposed to happen.
  -- Whenever this event will trigger, the widget will sample input from the text box, and run the monadic validation function
  -- provided as the next argument.
  -> (Event t Text -> m (Event t (ValidationResult Text a)))
  -- ^ The validation function. This function will receive an input event as an argument.
  -- This input event is based on the event generated by the function in the preceding argument.
  -> Text                       -- ^ The list id for showing a dropdown
  -> m ( Event t (Maybe a)
       , Dynamic t (Maybe a)
       )
uiTextInputAsync label inlineLabel (initval, initPopState) onSetName getTriggerEvent isValid dropdownListId = do
  (FormWidget v _ _, _) <- mkLabeledInput inlineLabel label (textFormWidgetAsync initPopState getTriggerEvent isValid dropdownListId) $ mkCfg initval
    & setValue .~ Just onSetName
  pure (updated v, v)
...
textFormWidgetAsync initPopState getTriggerEvent isValid dropdownListId cfg = mdo
  let
    triggerEv = tag (current inputDyn) $ getTriggerEvent inputE
