...
module Frontend.UI.Dialogs.SendCrossChain
  ( uiSendCrossChainModal
  ) where
...
-- | A modal for handling sending coin to a different chain.
uiSendCrossChainModal
  ...
  -> Event t ()
  -> m (mConf, Event t ())
uiSendCrossChainModal model (name, chain, acc) _onCloseExternal = do
  (conf, closes) <- fmap splitDynPure $ workflow $
    case _vanityAccount_unfinishedCrossChainTransfer $ _account_storage acc of
      Nothing -> sendCrossChainConfig model (InitialTransferData_Account (name, chain, acc))
...
previewTransfer model transfer = Workflow $ do
  ...
  (back, next) <- modalFooter $ do
    ...
    let isDisabled = pure $ isNothing crossChainData
    next <- confirmButton (def & uiButtonCfg_disabled .~ isDisabled) "Create Transaction"
    pure (back, next)

  let nextScreen = leftmost
        [ sendCrossChainConfig model (InitialTransferData_Transfer transfer) <$ back

        , case crossChainData of
            Nothing -> never
            Just ccd -> flip push next $ \() -> do
              ...
              let toAccount n = maybe (Left toTxBuilder) Right $
                    lookupAccountByTxBuilder toTxBuilder =<< Map.lookup n accounts
              ...
              pure $ ffor mNetInfo $ \n -> crossChainTransfer
                ...
                keys
                fromAccount
                (toAccount $ _sharedNetInfo_network n)
                fromGasPayer
                ccd
                amount
...
lookupAccountByTxBuilder
  :: TxBuilder
  -> Map AccountName (AccountInfo Account)
  -> Maybe (AccountName, ChainId, Account)
lookupAccountByTxBuilder ka accounts =
  (name, chain,) <$> accounts ^? ix name . accountInfo_chains . ix chain
  where
...
sendCrossChainConfig
  ...
  -> InitialTransferData
  -> Workflow t m (mConf, Event t ())
sendCrossChainConfig model initData = Workflow $ do
  close <- modalHeader $ text "Send Cross-chain"
  rec
  ...
  where
    ...
    mainSection currentTab = elClass "div" "modal__main" $ do
      (conf, useEntireBalance, txBuilder, amount) <- tabPane mempty currentTab SendCrossChainModalTab_Configuration $ do
        dialogSectionHeading mempty  "Destination"
        divClass "group" $ transactionDisplayNetwork model
        ...
        (txBuilder, amount, useEntireBalance) <- divClass "group" $ do
          ...
          let insufficientFundsMsg = "Sender has insufficient funds."
              ...
              recipientMustBeOnDiffChain = "Recipient must not be on chain " <> _chainId fromChain
              ...
              showTxBuilderPopover (_, (onInput, _)) = pure $ ffor onInput $ \case
                ...
                Right txb ->
                  if _txBuilder_accountName txb == fromName && _txBuilder_chainId txb == fromChain then
                    ...
                    PopoverState_Error recipientMustBeOnDiffChain
                  else
          ...
          let validatedTxBuilder = runExceptT $ do
                ...
                when (_txBuilder_chainId r == fromChain) $
                  throwError recipientMustBeOnDiffChain
                pure r
      ...
      mCaps <- tabPane mempty currentTab SendCrossChainModalTab_Sign $ do
        ...
        fmap join . holdDyn (pure Nothing) <=< dyn $ ffor dedrecipient $ \case
          ...
          Right dka -> do
            ...
            let gasPayerSection forChain mpayer accountPredicate mkPlaceholder = do
                  dyn_ $ ffor forChain $ \c ->
                    dialogSectionHeading mempty $ "Gas Payer" <> " (Chain " <> _chainId c <> ")"
            ...
            toGasPayer <- (fmap . fmap . fmap) Just $
    ...
    footerSection currentTab recipient mCaps = modalFooter $ do
      let (lbl, fanTag) = splitDynPure $ ffor currentTab $ \case
            SendCrossChainModalTab_Configuration -> ("Cancel", Left ())
            SendCrossChainModalTab_Sign -> ("Back", Right SendCrossChainModalTab_Configuration)
      ev <- cancelButton def lbl
      let (cancel, back) = fanEither $ current fanTag <@ ev
          (name, disabled) = splitDynPure $ ffor currentTab $ \case
            SendCrossChainModalTab_Configuration -> ("Next", fmap isLeft recipient)
            SendCrossChainModalTab_Sign -> ("Preview", fmap isNothing mCaps)
          cfg = def
      ...
      let nextTab = ffor (current currentTab <@ next) $ \case
            SendCrossChainModalTab_Configuration -> Just SendCrossChainModalTab_Sign
            SendCrossChainModalTab_Sign -> Nothing
        ...
        _ -> do
          ...
          case s of
            400 -- If the TX isn't reachable yet. This is fragile if chainweb changes the response message under us.
              | Just "SPV target not reachable: Target of SPV proof can't be reached from the source transaction"
                  <- _xhrResponse_responseText resp
              -- In this case we just need to wait a little longer. Push this request back
              -- to the end of the list
              -> failover (rs ++ [req])
            _ -> failover rs

data SendCrossChainModalTab
  = SendCrossChainModalTab_Configuration
  | SendCrossChainModalTab_Sign
  deriving (Eq, Ord, Show, Enum, Bounded)

displaySendModalTab :: DomBuilder t m => SendCrossChainModalTab -> m ()
displaySendModalTab = text . \case
  SendCrossChainModalTab_Configuration -> "Configuration"
  SendCrossChainModalTab_Sign -> "Sign"

makeTabs
  :: (DomBuilder t m, PostBuild t m, MonadHold t m, MonadFix m)
  => InitialTransferData -> Event t SendCrossChainModalTab -> m (Dynamic t SendCrossChainModalTab, Event t ())
makeTabs initData tabEv = do
  -- We assume that if there is a full transfer that we should head to the sign tab
  let initTab = initialTransferDataCata
        (const SendCrossChainModalTab_Configuration)
        (const SendCrossChainModalTab_Sign)
        initData
  ...
  rec
    ...
    (TabBar onTabClick) <- makeTabBar $ TabBarCfg
      { _tabBarCfg_tabs = [SendCrossChainModalTab_Configuration, SendCrossChainModalTab_Sign]
