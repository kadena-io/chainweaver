...
resizeDetectorWithAttrsAbsolute attrs w = do
  ...
  (parent, (expand, expandChild, shrink, w')) <-
    elAttr' "div" (Map.unionWith (<>) attrs ("style" =: "position: absolute;"))
      $ do
          w' <- w
          elAttr "div" containerAttrs $ do
            (expand, (expandChild, _)) <-
              elAttr' "div" containerAttrs
              $ elAttr' "div" ("style" =: childStyle)
              $ return ()
            (shrink, _) <-
              elAttr' "div" containerAttrs
              $ elAttr
  ...
  let
    p = uncheckedCastTo HTMLElement $ _element_raw parent
    reset = do
      let
        e = uncheckedCastTo HTMLElement $ _element_raw expand
    ...
    resetIfChanged ds = do
      ...
      if ds == (Just pow, Just poh) then return Nothing else fmap Just reset
  pb <- delay 0 =<< getPostBuild
  expandScroll <-
    wrapDomEvent
        (DOM.uncheckedCastTo DOM.HTMLElement $ _element_raw expand)
        (`on` Events.scroll)
      $ return ()
  shrinkScroll <-
    wrapDomEvent
        (DOM.uncheckedCastTo DOM.HTMLElement $ _element_raw shrink)
        (`on` Events.scroll)
      $ return ()
  ...
  rec
    resize <-
      performEventAsync
      $ fmap (\d cb -> (liftIO . cb) =<< liftJSM (resetIfChanged d))
      $ tag (current dimensions)
      $ leftmost [expandScroll, shrinkScroll]
    dimensions <- holdDyn (Nothing, Nothing)
      $ leftmost [size0, fmapMaybe id resize]
